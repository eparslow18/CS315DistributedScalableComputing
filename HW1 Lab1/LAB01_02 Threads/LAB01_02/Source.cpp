...
int main()
{
	...
	auto inCircle = 0; // counter for points following within circle
	auto totalPoints = 0; // counter for total number of pointers generated
	...
	/////////////////////EX01_02////////////////
	auto func= [&inCircle, &inCircleCount, &totalPoints](int tid) {
		double x = cs273_rand();
		double y = cs273_rand();
		if (WithinCircle(x, y))
			inCircle++;
	...
	for (int i = 0; i < 1e8 /10 ; i++) //take a tenth of the work 
		threadPool.push_back(thread(func, i));
	for (int i = 0; i < 10; i++)
		threadPool[i].join();
	///////////////////////////////////////////
